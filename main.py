...
def parse_opt():
    ...
    parser.add_argument('--batch_size', type=int, default=4, help='batch size (-1 for autobatch)')
    parser.add_argument('--epoch', type=int, default=13, help='epoch')
    ...
    # train_dataset, test_dataset = torch.utils.data.random_split(train_dataset, [0.8*len(train_dataset), len(train_dataset)-0.8*len(train_dataset)])
    # test_dataset = torchvision.datasets.Caltech101(root = "../datasets/caltech101",target_type='category',
    #                        train = False,
    ...
    if opt.class_balance:
        ...
        # class_weight = np.ones_like(np.unique(train_dataset.classes.__len__()))
